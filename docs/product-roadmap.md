...
5. **Continue policy hardening through the new APIs.** Preserve fail-closed origin/account/chain/prompt behavior; transaction approval is exposed only through `wallet.approveTransaction` with explicit dapp trigger, origin/account checks, optional chain verification, and capped target/value guardrails for zero-value or tightly scoped testnet flows.
